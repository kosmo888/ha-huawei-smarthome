...
本适配器暴露:
   ...
   3.  binary_sensor  防撬告警 (device_class: tamper)
...
# ---- 内置 Profile 缓存（解决官方 CDN 文件带 UTF-8 BOM 导致解析失败的底层问题） ----
...
# ---- 模块加载钩子：自动补齐本地缓存与运行时 Context -----------------
try:
    for base in ["/config/.storage/huawei_smarthome/profiles", ".storage/huawei_smarthome/profiles"]:
        try:
            os.makedirs(base, exist_ok=True)
            target_f = os.path.join(base, "21RM.json")
            wrapped = {
                "version": 1,
                "minor_version": 1,
                "key": "huawei_smarthome/profiles/21RM.json",
                "data": {"profile": _FALLBACK_PROFILE}
            }
            with open(target_f, "w", encoding="utf-8") as f:
                json.dump(wrapped, f, ensure_ascii=False)
            _LOGGER.info("Successfully wrote 21RM.json to %s", target_f)
        except Exception:
            pass

    ctx_cls = getattr(sys.modules.get("custom_components.huawei_smarthome.device_adapters.context"), "DeviceContext", None)
    if ctx_cls is not None:
        for obj in gc.get_objects():
            if isinstance(obj, ctx_cls):
                desc = getattr(obj, "descriptor", None)
                p_id = getattr(desc, "prod_id", "")
                p_name = getattr(desc, "name", "")
                if str(p_id).upper() == "21RM" or "德施曼" in str(p_name):
                    obj.profile = _FALLBACK_PROFILE
                    obj.adapter = ADAPTER
                    _LOGGER.info("Successfully patched in-memory DeviceContext for 21RM lock!")
except Exception as err:
    _LOGGER.debug("21RM hook exception: %s", err)
